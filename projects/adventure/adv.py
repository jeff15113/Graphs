...
graph = {}
unknown_room = {}
graph[0] = player.currentRoom.getExits()
for exit in graph[0]:
    unknown_room[exit] = '?'
    graph[0] = unknown_room

oppositeLookup = {'n': 's', 's': 'n', 'e': 'w', 'w': 'e'}
...
def bfs(start):
    ...
    while queue.size() > 0:
        ...
        if lastItemInPath not in visited:
            for exit in graph[lastItemInPath]:
                if graph[lastItemInPath][exit] == '?':
                    return path
            visited.add(lastItemInPath)
            for new_exit in graph[lastItemInPath]:
                path_copy = path.copy()
                path_copy.append(graph[lastItemInPath][new_exit])
...
while True:
    ...
    if len(directionList) > 0:
        exitDirection = directionList[0]
        traversalPath.append(exitDirection)
        oldRoomId = player.currentRoom.id
        ...
        if player.currentRoom.id not in graph:
            for exit in player.currentRoom.getExits():
                unknownRoom[exit] = '?'
            graph[player.currentRoom.id] = unknownRoom
        graph[oldRoomId][exitDirection] = player.currentRoom.id
        graph[player.currentRoom.id][oppositeLookup[exitDirection]] = oldRoomId
    else:
        walkDirections = bfs(player.currentRoom.id)
        if len(walkDirections) == 0:
            break
        for direction in getDirectionsFromPath(walkDirections):
